...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['VAT_SENSE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['VAT_SENSE_LOG'] or 'warn' if it isn't set.
   */
...
export class VatSense {
   ...
   * API Client for interfacing with the Vat Sense API.
   *
   * @param {string | undefined} [opts.username=process.env['VAT_SENSE_USERNAME'] ?? undefined]
   * @param {string | undefined} [opts.password=process.env['VAT_SENSE_PASSWORD'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['VAT_SENSE_BASE_URL'] ?? https://api.vatsense.com/1.0] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('VAT_SENSE_BASE_URL'),
    username = readEnv('VAT_SENSE_USERNAME'),
    password = readEnv('VAT_SENSE_PASSWORD'),
    ...opts
  }: ClientOptions = {}) {
    ...
    const options: ClientOptions = {
      ...
      password,
      ...opts,
      baseURL: baseURL || `https://api.vatsense.com/1.0`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? VatSense.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('VAT_SENSE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.vatsense.com/1.0';
  }
   ...
   */
  currency: API.Currency = new API.Currency(this);
  /**
   * VAT-compliant invoice management
   */
  invoice: API.InvoiceResource = new API.InvoiceResource(this);
  /**
...
VatSense.Rates = Rates;
VatSense.Countries = Countries;
VatSense.Validate = Validate;
VatSense.Currency = Currency;
VatSense.InvoiceResource = InvoiceResource;
VatSense.Usage = Usage;
