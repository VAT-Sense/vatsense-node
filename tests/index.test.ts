...
import { APIPromise } from 'vat-sense/core/api-promise';

import util from 'node:util';
import VatSense from 'vat-sense';
import { APIUserAbortError } from 'vat-sense';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['VAT_SENSE_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: VatSense) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new VatSense({ username: 'My Username', password: 'My Password' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['VAT_SENSE_LOG'] = 'debug';
    const client = new VatSense({
  logger: logger,
  username: 'My Username',
  password: 'My Password',
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['VAT_SENSE_LOG'] = 'not a log level';
    const client = new VatSense({
  logger: logger,
  username: 'My Username',
  password: 'My Password',
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'VAT_SENSE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  password: 'My Password',
    ...
    afterEach(() => {
      process.env['VAT_SENSE_BASE_URL'] = undefined;
    });
  ...
  password: 'My Password',
    ...
    test('env variable', () => {
      process.env['VAT_SENSE_BASE_URL'] = 'https://example.com/from_env';
      const client = new VatSense({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['VAT_SENSE_BASE_URL'] = ''; // empty
      const client = new VatSense({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.vatsense.com/1.0')
    });

    test('blank env variable', () => {
      process.env['VAT_SENSE_BASE_URL'] = '  '; // blank
      const client = new VatSense({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.vatsense.com/1.0')
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['VAT_SENSE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new VatSense({ username: 'My Username', password: 'My Password' });
  ...
  password: 'My Password',
    ...
    // default
    const client2 = new VatSense({ username: 'My Username', password: 'My Password' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['VAT_SENSE_USERNAME'] = 'My Username';
    process.env['VAT_SENSE_PASSWORD'] = 'My Password';
    const client = new VatSense();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['VAT_SENSE_USERNAME'] = 'another My Username';
    process.env['VAT_SENSE_PASSWORD'] = 'another My Password';
    const client = new VatSense({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
