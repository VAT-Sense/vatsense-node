...
```sh
npm install git+ssh://git@github.com:stainless-sdks/vat-sense-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install vat-sense`
...
const client = new VatSense({
  ...
  password: process.env['VAT_SENSE_PASSWORD'], // This is the default and can be omitted
});
...
const rates: VatSense.RateListResponse = await client.rates.list();
```
...
<!-- prettier-ignore -->
```ts
const rates = await client.rates.list().catch(async (err) => {
...
const response = await client.rates.list().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: rates, response: raw } = await client.rates.list().withResponse();
...
1. Via the `VAT_SENSE_LOG` environment variable
...
const client = new VatSense({
  logger: logger.child({ name: 'VatSense' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/vat-sense-typescript/issues) with questions, bugs, or suggestions.
